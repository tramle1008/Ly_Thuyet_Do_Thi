// đồ thị đơn, vô hướng, bfs in ra các bộ phận liên thông
#include <stdio.h>
#define max_vertices 20
#define max_lenght 20
#define max_element 40
typedef struct
{
    int A[max_vertices][max_vertices];
    int n;
} Graph;
void init_graph(Graph *G, int n)
{
    int i;
    G->n = n;
    for (i = 1; i <= n; i++)
    {
        for (int j = 1; j <= n; j++)
            G->A[i][j] = 0;
    }
}

// them cung vao do thi
void add_adge(Graph *G, int x, int y)
{
    if (x == y)
    {
        G->A[x][y] += 1;
    }
    else
    {
        G->A[x][y] += 1;
        G->A[y][x] += 1;
    }
}

// khai bao cau truc danh sach List
typedef struct
{
    int date[max_lenght];
    int size;
} List;
// khoi tao List rong
void make_null(List *list)
{
    list->size = 0;
}
// them vao mot phan tu
void push_back(List *list, int x)
{
    list->date[list->size] = x;
    list->size++;
}
// lay mot phan tu trong list tai di tri i
int element_at(List *list, int i)
{
    return (list->date[i - 1]);
}
List neighbors(Graph *G, int x)
{
    List L;
    make_null(&L);
    for (int i = 1; i <= G->n; i++)
    {
        if (G->A[x][i] == 1)
            push_back(&L, i);
    }
    return L;
}
// khai bao hang doi
typedef struct
{
    int data[max_element];
    int front; // chi so dau hang doi _ lay data
    int rear;  // chi so cuoi hang doi _ them data
} Queue;
// khoi tao hang doi rong
void make_null_queue(Queue *q)
{
    q->front = 0;
    q->rear = -1;
}
// them mot phan tu
void push_queue(Queue *q, int x)
{
    q->rear++;
    q->data[q->rear] = x;
}
// kiem tra hang doi co rong khong
int empty_queue(Queue *q)
{
    return (q->front > q->rear);
}
// lay mot phan tu o dau hang doi
int top_queue(Queue *q)
{
    return (q->data[q->front]);
}
void pop_queue(Queue *q)
{
    q->front++;
}
// ham duyet theo chiều rộng từ đih r
int mark_bfs[100];

List bfs(Graph *G, int s, int mark[])
{
    Queue Q;
    make_null_queue(&Q);
    List lbfs;
    make_null(&lbfs);
    push_queue(&Q, s);
    while (!empty_queue(&Q))
    {
        int u = top_queue(&Q);
        pop_queue(&Q);
        if (mark[u] == 1)
            continue;
        //   printf("Duyet: %d\n", u);
        push_back(&lbfs, u);
        mark[u] = 1;
        List list;
        make_null(&list);
        list = neighbors(G, u);
        int i;
        for (i = 1; i <= list.size; i++)
        {
            int v = element_at(&list, i);
            if (mark[v] == 0)
            {
                push_queue(&Q, v);
            }
        }
    }
    return lbfs;
}

// Duyet bang tu dinnh R
int main()
{

    Graph G;
    int e, n, m;

    freopen("dt.txt", "r", stdin);
    scanf("%d%d", &n, &m);
    // m=m-1;
    init_graph(&G, n);
    int u, v;
    for (e = 1; e <= m; e++)
    {
        scanf("%d%d", &u, &v);
        add_adge(&G, u, v);
    }
    int i;
    for (int t = 1; t <= n; t++)
    {
        mark_bfs[t] = 0;
    }
    List l = bfs(&G, 6, mark_bfs);
    for (i = 1; i <= l.size; i++)
    {
        v = element_at(&l, i);
        printf("%d ", v);
        mark_bfs[v] = 1; // danh dau da duyet
    }
    printf("\n");
    for (u = 1; u <= n; u++)
    {
        if (mark_bfs[u] == 0)
        {
            // kiem tra dinh duyet chua

            List Li = bfs(&G, u, mark_bfs);
            // ta có thể cài đặt các thuật toán sắp xếp nếu muốn ở đây, hoặc nếu chỉ cần tìm số bộ phận liên thông thì s++; ở đây
            for (i = 1; i <= Li.size; i++)
            {
                v = element_at(&Li, i);
                printf("%d ", v);
                mark_bfs[v] = 1; // danh dau da duyet
            }
            printf("\n");
        }
    }
    return 0;
}